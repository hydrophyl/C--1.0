...
{
	const int xOffset = 12;					// linker Rand f�r Werte und Achse
	...
	ofstream output;
	output.open("Plot_zu_DatenEinlesen3.txt");
	...
	***************************/
	...
	/* Lage der x-Achse bestimmen
	   es wird der y-Wert genommen, welcher gleich Null oder der Null an n�chsten ist 
	...
	/* Position der y-Achse bestimmen
		es wird der x-Wert genommen, welcher gleich Null oder der Null an n�chsten ist */
	...
	// Testausgabe
	//for (int y=0;y<yAnzahl;y++) output << y << " " << yAchse[y] << endl;


	/* Ausgabe der Koordinatenachsen:
	   es werden immer die Achsen am Rand der Flaeche gezeichnet, weil dort die Einheiten stehen.
	   Wenn ein Vorzeichenwechsel vorliegt, wird die jeweilige Achse zus�tzlich eingezeichnet. 
	*/

	// x-Achse am unteren Rand f�r Beschriftung immer ausgeben
	if(xVorzWechsel)
	{
	...
	// y-Achse am linken Rand f�r Beschriftung immer ausgeben
	if(yVorzWechsel)
	{
	...
	*/
	...
	for(int y=yAnzahl-1;y>=0;y--)
	{
		...
		{
			output << setw(xOffset-2) << right << setprecision(2) << fixed << yAchse[y] << " ";
		}
		else
		{
			output << setw(xOffset-1) << " ";
		}

		// Zeichenfl�che ausgeben
		for(int x=0;x<xAnzahl;x++)
		{
			output << Graphik[x][y];
		}
	...
	for (int x=0;x<xAnzahl;x++)
	{
		...
		{
			output << "\b\b\b" << setw(5) << left << setprecision(2) << fixed << xWerte[x]; 
		}
		else
		{
			output << setw(1) << " " ; 
